...
def get_similarity_of_attractor_spaces(attractors1,attractors2,basin_sizes1,basin_sizes2):
    ...
    for attractor,basin_size in zip(attractors1,basin_sizes1):
        if type(attractor) in [int,np.int64,np.int32,np.int16,np.float64]:
            attractor = [attractor]
...
if __name__ == '__main__':
    ...
    for ASYNCHRONOUS in [True,False]:    
        ...
        for ii,i in enumerate(good_indices):
            ...
            if max_N <= 20 and COMPUTE_EXACT_STATE_SPACE:
                ...
                for j in range(min(2**n_constants[i],max_n_inputs_simulated_in_attractor_computation)):
                    if ASYNCHRONOUS:
                        dummy = can.get_attractors_asynchronous(F, I, len(F),nsim=n_IC_per_network_in_attractor_computation, 
                                                                   initial_sample_points=initial_sample_points[ii][j], 
                                                                   INITIAL_SAMPLE_POINTS_AS_BINARY_VECTORS=False,
                                                                   search_depth=search_depth_asynchronous, record_after_this_many_updates = 100, SEED=-1, DEBUG=False)
                        attractors[-1].append(dummy[2])
                        basin_sizes[-1].append(dummy[4])
                        #state_spaces[-1].append(dummy[6]) 
                        n_timeout[-1].append(0)
                    else:
        ...
        file = open('bio_data_EXACT%i_ASYNCHRONOUS%i' % (int(COMPUTE_EXACT_STATE_SPACE),int(ASYNCHRONOUS)), 'wb')
        pickle.dump([good_indices,attractors,basin_sizes,state_spaces,n_timeout], file)
        file.close()
        ...
        formalisms = ['Ising','Isingmod','01','01mod']
        for func_threshold,formalism in zip(func_thresholds,formalisms):
            ...
            for ii,i in enumerate(good_indices):
                ...
                if max_N <= 20 and COMPUTE_EXACT_STATE_SPACE:
                    ...
                    for j in range(min(2**n_constants[i],max_n_inputs_simulated_in_attractor_computation)):
                        if ASYNCHRONOUS:
                            dummy = can.get_attractors_asynchronous(F, I, len(F),nsim=n_IC_per_network_in_attractor_computation, 
                                                                       ...
                                                                       INITIAL_SAMPLE_POINTS_AS_BINARY_VECTORS=False,
                                                                       search_depth=search_depth_asynchronous, record_after_this_many_updates = 100, SEED=-1, DEBUG=False)
                            attractors_threshold[-1].append(dummy[2])
                            basin_sizes_threshold[-1].append(dummy[4])
                            #state_spaces_threshold[-1].append(dummy[6]) 
                            n_timeout_threshold[-1].append(0)                    
                        else:
                            dummy = can.get_attractors_synchronous(F, I, nsim = n_IC_per_network_in_attractor_computation, 
                                                         n_steps_timeout=n_steps_timeout_when_computing_attractors, 
                                                         initial_sample_points = initial_sample_points[ii][j],
                    ...
                    print(i)
                    if ASYNCHRONOUS:
                        jsd = np.mean([get_similarity_of_attractor_spaces(a1,a2,b1,b2)[0] for a1,a2,b1,b2 in zip(attractors[ii],attractors_threshold[ii],basin_sizes[ii],basin_sizes_threshold[ii])])
                        jsd_attractors.append( jsd )
                        file = open('jsd_EXACT%i_ASYNCHRONOUS%i_%s_ID%i' % (int(COMPUTE_EXACT_STATE_SPACE),int(ASYNCHRONOUS),formalism,i), 'w')
                        file.write(str(jsd))
                        file.close()
    ...
    CC_ONLY=True
    if CC_ONLY:
        indices_of_good_indices_in_CC = np.array([' ' in models_loaded[ID] for ID in good_indices])
    ...
    for metrics,metrics_label in zip([all_overlap_synchronous_state_space_def2,all_overlap_synchronous_state_space,1-np.array(all_jsd_attractors)],
        ...
        for jj,(func_threshold,metric) in enumerate(zip(func_thresholds,metrics)):
            suffix = ''.join(func_threshold.__name__.split('_')[3:])   
            if CC_ONLY:
                ax.plot(np.sort(metric[indices_of_good_indices_in_CC]),label=formalisms[jj],color=colors[jj],ls=lss[jj])
                print(metrics_label,formalisms[jj],round(np.mean(metric[indices_of_good_indices_in_CC]),3))
            else:
                ax.plot(np.sort(metric),label=formalisms[jj],color=colors[jj],ls=lss[jj])
                print(metrics_label,formalisms[jj],round(np.mean(metric),3))
                
        ax.legend(loc='best',frameon=False,ncol=2)
        ax.set_ylabel(metrics_label)
        ax.set_xlabel('networks')
        plt.savefig('cdf_%s.pdf' % (metrics_label.replace(' ','_') + ('CC_ONLY' if CC_ONLY else '')),bbox_inches='tight')
    ...
    for ii,(metrics,metrics_label) in enumerate(zip([all_overlap_synchronous_state_space_def2,all_overlap_synchronous_state_space,1-np.array(all_jsd_attractors)],
        ...
        for jj,(func_threshold,metric) in enumerate(zip(func_thresholds,metrics)):
            suffix = ''.join(func_threshold.__name__.split('_')[3:])  
            if CC_ONLY:
                ax[ii].plot(np.sort(metric[indices_of_good_indices_in_CC]),label=formalisms[jj],color=colors[jj],ls=lss[jj])
                print(metrics_label,formalisms[jj],np.mean(np.sort(metric[indices_of_good_indices_in_CC]))*100)
            else:
        ...
        ax[ii].set_xlabel('Cell Collective networks' if CC_ONLY else 'biological networks')
        if ii==0:
            ax[ii].set_ylim([0.25,ax[ii].get_ylim()[1]])
    plt.subplots_adjust(wspace=0.35)
    plt.savefig('cdf_all%s.pdf' % (('_CC_ONLY' if CC_ONLY else '')),bbox_inches='tight')
    
    
    all_jsd_attractors_async = []
    for func_threshold,formalism in zip(func_thresholds,formalisms):
        all_jsd_attractors_async.append([])
        for ii,i in enumerate(good_indices):
            f = open('jsd_EXACT0_ASYNCHRONOUS1_%s_ID%i' % (formalism,i),'r')
            all_jsd_attractors_async[-1].append(float(f.read()))
            f.close()
    all_jsd_attractors_async = np.array(all_jsd_attractors_async)
    ...
    f,ax = plt.subplots(1,3,figsize=(7.5,2))
    for ii,(metrics,metrics_label) in enumerate(zip([1-np.array(all_jsd_attractors),1-np.array(all_jsd_attractors),1-np.array(all_jsd_attractors)],
                                                    ['overlap synchronous\nattractor space (Eq. 12)','','overlap asynchronous\nattractor space'])):
    ...
    plt.subplots_adjust(wspace=0.35)
    plt.savefig('cdf_all%s_ASYNCHRONOUS%i.pdf' % (('_CC_ONLY' if CC_ONLY else ''),int(ASYNCHRONOUS)),bbox_inches='tight')
                
    
    
    names_y = ['overlap async. state space','overlap sync. state space','overlap async. attractor space','overlap sync. attractor space']   
    names_x = names_y + ['network size','average network connectivity'] 

    spearman_mat = np.ones((4*len(formalisms),len(names_x)))
    spearman_mat_p = np.ones((4*len(formalisms),len(names_x)))
    pearson_mat = np.ones((4*len(formalisms),len(names_x)))
    for ii,formalism in enumerate(formalisms):
        metrics_x = [all_overlap_synchronous_state_space_def2[ii],all_overlap_synchronous_state_space[ii],1-all_jsd_attractors_async[ii],1-all_jsd_attractors[ii]]
        ...
        for i,data_x in enumerate(metrics_x):
            for j,data_y in enumerate(metrics_y):
                spearman_mat[4*ii+i,j] = stats.spearmanr(data_x,data_y)[0]
                spearman_mat_p[4*ii+i,j] = stats.spearmanr(data_x,data_y)[1]
                pearson_mat[4*ii+i,j] = stats.pearsonr(data_x,data_y)[0]
    ...
    for ii,(data,name) in enumerate(zip([spearman_mat,pearson_mat],['spearman','pearson'])):
        ...
        im = ax.imshow(data,origin='upper',vmin=-1,vmax=1,cmap=matplotlib.cm.RdBu)
        ax.set_yticks(range(4*len(formalisms)))
        ...
        for jj in range(4):
            ax.plot([-.5,5.5],[4*(jj+1)-.5,4*(jj+1)-.5],'k-',lw=0.5)
        for jj,formalism in enumerate(formalisms):
            ax.text(-10,1+4*jj,formalism,va='center',ha='center',rotation=90)
            ax.plot([-9.5]*2,[-.3+4*jj,-.3+3.6+4*jj],'k-',lw=0.5,clip_on=False)
        ax.set_xlim([x1,x2])
...
#        for jj,(name_metric,metric) in enumerate(zip(names_y,[all_overlap_synchronous_state_space_def2,all_overlap_synchronous_state_space,all_jsd_attractors])):
        for jj,(name_metric,metric) in enumerate(zip(names_y,[all_jsd_attractors,all_jsd_attractors,all_jsd_attractors])):
    ...
    #count the frequency of specific functions
    degree = 4
    AUTOREGULATION = np.array([el != el.lower() for el in res])
    res_f_str = np.array([''.join(list(map(str,el))) + ('A' if AUTO else 'R') for el,AUTO in zip(res_f,AUTOREGULATION)])
    counts = pd.Series(np.array(res_f_str)[len_res==degree]).value_counts()
    out = []
    for key,val in zip(counts.index,counts.values):
        indices = np.where(np.bitwise_and(AUTOREGULATION==(True if key[-1]=='A' else False),res_f_str == key))[0]
        out.append([key,val])
        if key[-1]=='R':
            depth,n_layers,can_inputs,can_outputs,core_polynomial,can_order = can.find_layers(res_f[indices][0])
            layer_structure = can.get_layerstructure_given_canalizing_outputs_and_corefunction(can_outputs,core_polynomial)
            dummy = []
            count = 0
            for r in range(n_layers):
                n_act = sum(can_inputs[count:count+layer_structure[r]] == can_outputs[count:count+layer_structure[r]])
                n_inh = layer_structure[r] - n_act
                count += layer_structure[r]
                dummy.append(''.join(['+'] * n_act + ['-'] * n_inh))
            layer_structure_with_act_vs_inh_info = ','.join(dummy)
            out[-1].extend([depth,n_layers,layer_structure,list(can_inputs),list(can_outputs),layer_structure_with_act_vs_inh_info])
        else:
            out[-1].extend([0,0,0,0,0,0])            
        for i in range(n_threshold_formalisms):
            out[-1].append(np.mean(1-np.array(differences_f_to_threshold[i])[indices]))
    A = pd.DataFrame(out)
    A.to_excel('out_ncf_degree%i.xlsx' % degree)
    ...
    for w in range(1,2**n,2):
        r,kis = can.get_layerstructure_of_an_NCF_given_its_Hamming_weight(n,w)
        ranges = [list(range(k_i+1)) for k_i in kis]
        for dummy in itertools.product(*ranges):
            input_types = ''.join(['+'*el + '-'*(k_i-el) for el,k_i in zip(dummy,kis)])
            first_output = int(w>2**(n-1))
            f = can.random_k_canalizing_with_specific_layerstructure_inputtypes_outputs(n, kis, input_types = input_types, first_output = first_output)
            similarities = []
            f_ts = []
            for jj,threshold_formalism in enumerate(threshold_formalisms):
                f_t = threshold_formalism(f)[0]
                similarity = get_similarity_functions(f,f_t)
                f_ts.append(f_t)
                similarities.append(similarity)
            similarities = np.array(similarities)
            print(w,kis,input_types,first_output,''.join(map(str,f)),np.round(similarities,3))
            max_similarities = np.array([1-abs(np.mean(f) - np.mean(f_t)) for f_t in f_ts])
            delta_similarities = max_similarities - similarities
            out.append([w,kis,input_types,first_output,''.join(map(str,f))])
            out[-1].extend([''.join(map(str,f_t)) for f_t in f_ts])
            out[-1].extend([sum(f_t) for f_t in f_ts])
            out[-1].extend(list(similarities))
            out[-1].extend(list(delta_similarities))
    A = pd.DataFrame(out)
    A.to_excel('out_ncf_degree%i_all.xlsx' % n)
    ...
    for w in range(1,2**n,2):
        ...
        for dummy in itertools.product(*ranges):
            ...
            similarities = []
            f_ts = []
            for jj,threshold_formalism in enumerate(threshold_formalisms[2:3]):
                f_t = threshold_formalism(f)[0]
                similarity = get_similarity_functions(f,f_t)
                f_ts.append(f_t)
                similarities.append(similarity)
            ...
            out.append([w,kis,input_types,first_output,''.join(map(str,f))])
            out[-1].extend([''.join(map(str,f_t)) for f_t in f_ts])
            out[-1].extend(list((np.array([val1-val2+1 for val1,val2 in zip(f,f_t[:2**n])]) + np.array([val1-val2+1 for val1,val2 in zip(f,f_t[2**n:])]))/2))
            out[-1].extend([sum(f_t) for f_t in f_ts])
            out[-1].extend(list(similarities))
            out[-1].extend(list(delta_similarities))
    A = pd.DataFrame(out)
    A.to_excel('out_ncf_degree%i_only01.xlsx' % n)

    for n in range(3,6):
        for n_act in range(n+1):
            threshold_formalism = threshold_formalisms[2]
            f_t = threshold_formalism(f,type_of_each_regulation = ['increasing'] * n_act + ['decreasing'] * (n-n_act))[0]
            print(n,n_act,sum(f_t))

    
    #don't care about bio counts, just look at all combinations of NCFs of a fixed degree
    #stratify by absolute bias and number of positive vs negative regulators, don't consider auto-regulation
    n = 3
    nsim = 100
    ...
    for w in range(1,2**n,2):
        for _ in range(nsim):
            r,kis = can.get_layerstructure_of_an_NCF_given_its_Hamming_weight(n,w)
            f = can.random_k_canalizing_with_specific_layerstructure(n,kis)
            type_of_each_regulation = []
            (NONDEGENERATED,monotonic) = can.is_monotonic(f,True)
            for el in monotonic:
                type_of_each_regulation.append(el)
            #print(f,type_of_each_regulation.count('increasing'),[get_similarity_functions(f,threshold_formalism(f,type_of_each_regulation=type_of_each_regulation)[0]) for threshold_formalism in threshold_formalisms])
            for jj,threshold_formalism in enumerate(threshold_formalisms):
                similarity = get_similarity_functions(f,threshold_formalism(f,type_of_each_regulation=type_of_each_regulation)[0])
                ax[jj].text(w,similarity,str(type_of_each_regulation.count('increasing')),ha='center',va='center')
    for jj,threshold_formalism in enumerate(threshold_formalisms):
        ax[jj].set_ylim([0,1])
        ax[jj].set_xlim([0,2**n])
...
for degree in range(4,5):
    ...
    mean_similarities_ising_ising_mod.append(np.mean(similarities_ising_ising_mod))
